...
<c:set var="memberID" value="${sessionScope}" />
...
<!-- 	<div id="jb-container"> -->
<div id="jb-header">
	<h1 style="font-size: 2em">${article.p_title}</h1>
</div>
<div id="jb-content">
	<p>${article.p_detail} </p>

</div>
<div id="jb-sidebar-right">
	<h1 class="a" style="font-size: 4em">경매종료일</h1>
	<h1 id="d-day" style="font-size: 2em">${article.p_date}</h1>
					...
					<p style="font-size: 2em">${article.p_sprice}원</p>
						...
						<div class="myipchal">
							...
							<h3 class="a" style="font-size: 2em">입찰가
								...
								<form action="itemAddPriceAcion.it?it_no=${article.p_num}&memberID=${article.m_id}
								&customerID=${customerID }&endTime=${article.p_eprice}
								&nowPrice=${article.a_price}" method="post" id="ipchal">
...
<script type="text/javascript">
		...
		// 타이머 제어문
		var countDownDate = new Date("2022-12-22 12:30").getTime();
		...
		var x = setInterval(function() { //1초마다 갱신되도록 함수 생성,실행
			...
			if(distance<=0){
				...
				$.ajax({
	                    ...
	                    type:"post",
	                    dataType:"html",
	                    data:{"it_no":${article.p_num},"memberID":"${article.m_id}","endTime":"${article.p_date}"},
