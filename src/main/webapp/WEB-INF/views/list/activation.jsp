		...
		<span class="title">출품중인 상품</span>
				...
				<c:forEach var="i" items="${selectItemExhibiting }">
					<c:url value="/item/view.it" var="itemExhibiting">
						<c:param name="no" value="${i.p_num }" />
					</c:url>
					<a href="${itemExhibiting }" class="item_list">
						<span class="thumbnail"><img src="../upload/${i.p_img1 }"></span>
						<span class="name"><c:out value="${i.p_title }" /></span>
						<span class="price_start">시작가 <c:out value="${i.a_price }" />원</span>
					</a>
				</c:forEach>
